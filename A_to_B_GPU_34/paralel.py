...
def synchronize_models(agent, weights, device=None):
    ...
    """
    n = len(weights)
    model_paths = [model_incr_load + "-{}".format(i) for i in range(n)]
    ...
    for p in model_paths:
        sd = agent.load(p, map_location=device)
...
def handle_crash_worker(rank: int, world_size: int, shared_array: mp.Array, results_queue: mp.Queue):
    """
    ...
    try:
        ...
        agent = DeepActorCriticAgent(device=device)
        agent.mean_reward = 0
        agent.episode = 0

        # If a partial checkpoint exists, load it
        if os.path.isfile(model_incr_load + "-{}".format(rank)):
            agent.load(model_incr_load + "-{}".format(rank))
        else:
            if rank == 0:
        ...
        print(f"[Rank {rank}] Exception inside handle_crash_worker: {e}")
        raise
