...
class DeepActorCriticAgent(mp.Process):
    ...
    def get_action(self, obs, speed, manouver, testing=False):
        ...
        action_distribution = self.policy(obs, speed,
    ...
    def save(self, name):
        ...
        with lock:
            ...
            if logging:
                print(
                    f'Model path exists {os.path.exists(model_file_name)} model up dir exists {os.path.exists("/".join(model_file_name.split("/")[:-1]))}')
...
def synchronize_models(agent, weights, device=None):
    """
    Function for synchronization model by averaging weights by mean_reward, with weighted average.
    """
    n = len(weights)
    ...
    for p in model_paths:
        sd = agent.load(p)
        # if your .pth contains a full model, swap in:  sd = sd.state_dict()
        state_dicts.append(sd)

    # 2) Build averaged state_dict
    avg_sd = {}
    # start with zeros of same shape/dtype as first
    for k, v in state_dicts[0].items():
        avg_sd[k] = torch.zeros_like(v, device=device)

    # accumulate weighted sum
    for weight, sd in zip(w, state_dicts):
        for k, v in sd.items():
            avg_sd[k] += weight * v.to(device)

    # 3) Overwrite each original .pth with the averaged state_dict
    for p in model_paths:
        agent.save(p.replace('.pth', ''))

    print(f"Synchronized {n} models and saved averaged weights back to each file.")


def handle_crash(rank: int, world_size: int, shared_array: mp.Array, results_queue: mp.Queue):
    """
    This wrapper will be used as the target function for each GPU‐process.
    It pins to GPU=rank, sets up wandb, instantiates the agent (whose networks
    will live on cuda:rank), and then enters your main loop. If it ever
    completes all episodes, it does results_queue.put(1). If anything throws,
    it re‐raises so that exitcode != 0.
    """
    try:
        ...
        if logging:
            print(f"[Rank {rank}] Beginning Weights & Biases initialization…")
            wandb.init(
                project="A_to_B",
                resume="allow",
            ...
            wandb.run.notes = (
                "Img+speed+manouver. speed/100. Nowy model (nie ten z blokami rezydualnymi), "
                "z predkoscia oraz manewrem na wejsciu. Scenariusz 10 - randomowa trasa w kazdym epizodzie. "
                "Slight turns like: 9: [0, 1, 0.2], #brake slight right. Gradients logged. Stara funkcja nagrody.\n"
                "speed_reward = -1.2 + speed/3"
                "if route_distance < 1:"
                "   route_distance_reward = 1"
                "else:"
                "   route_distance_reward = -speed/3. "
                "Startowanie w dwóch miejscach na przemian"
            )
        print(f' --- Primary port {primary_port}  and rank {rank}---')
        print(f' --- Creating agent for server on port {(int(rank) * 1000 + primary_port)} ---')
        agent = DeepActorCriticAgent(port=(int(rank) * 1000 + primary_port))
        agent.mean_reward = 0
        agent.episode = 0

        # If a partial checkpoint exists, load it
        load_path_for_this_process = model_incr_load.replace('.pth', "-{}.pth".format(rank))
        if os.path.isfile(load_path_for_this_process):
            agent.load(load_path_for_this_process)
            print(f'Loading model for process {rank} and episode is {agent.episode}')
        else:
            if rank == 0:
                print(f"[Rank {rank}] No existing model checkpoint found: starting fresh.")
        ...
        while True:
            ...
            ep_reward = 0.0
            step_num = 0
            actions_counter = {name: 0 for name in ac.ACTIONS_NAMES.values()}
            episode_step = 0
            ...
            while not done:
                ...
                on_junction, left_junction = agent.environment.planner.on_junction(
                    agent.environment.vehicle.get_location()
                )
                if left_junction:
                    try:
                        i += 1
                ...
                action = agent.get_action(state_rgb, speed_tensor, manouver_tensor, testing=False)

                if save_image:
                    agent.environment.state_observer.action = action
                    agent.environment.state_observer.step = episode_step
                ...
                # Wait one world tick (trade‐off: you may need two ticks if you want to ensure sensors update)
                ...
                save_image = True if agent.episode in episodes_to_save_images else False
                new_state, reward, done, route_distance, speed_tensor, distance_from_goal = agent.environment.step(
                    save_image=save_image,
                    episode=agent.episode,
                    step=episode_step
                )
                ...
                if not False and (step_num >= 5 or done):
                    actor_loss, critic_loss, actor_lr, critic_lr = agent.optimize(
                        new_state, done, speed_tensor, manouver_tensor
                    )
                    step_num = 0
            ...
            agent.mean_reward = (
                                        agent.mean_reward * (min(100, agent.episode) - 1) + ep_reward
                                ) / min(100, agent.episode)
            if ep_reward > agent.best_reward:
                agent.best_reward = ep_reward

            # Save an incremental checkpoint
            print(f'Saving model for process {rank} with episode {agent.episode}')
            ...
            if logging:
                print(f"[Rank {rank}] Logging episode {agent.episode} stats…")
                wandb.log({
                    "episode steps": episode_step,
                    "episode duration [s]": episode_time.seconds,
                    "reward": ep_reward,
                    "episode": agent.episode,
        ...
        # If we exit the while‐loop normally (episode >= 10000), _that_ is a “successful finish.”
        # So we do results_queue.put(1) and then return cleanly.
        results_queue.put(1)

        # Clean up any CARLA client/world references
        try:
            del agent.environment.world
            del agent.environment.client
        except Exception:
            pass
        return

    except Exception as e:
        # If any exception (including CARLA server crash or a CUDA OOM) occurred,
        # we want this process’s exitcode != 0. So we print a short message and re-raise.
        print(f"[Rank {rank}] Exception inside handle_crash: {e}")
        raise


def training_process(rank, world_size, shared_array, results_queue):
    """
    Start one new training process for
    """
    p = mp.Process(
        target=handle_crash,
        args=(rank, world_size, shared_array, results_queue),
        daemon=False
    )
    p.start()
    print(f"[Supervisor] Started worker for rank {rank} with PID {p.pid}")
    return p
...
if __name__ == "__main__":
    ...
    # Shared state
    shared_array = mp.Array('d', [0.0] * world_size)
    results_queue = mp.Queue()

    # Track processes per rank
    processes = {}

    # Launch all workers initially
    for rank in range(world_size):
        processes[rank] = training_process(rank, world_size, shared_array, results_queue)

    # Supervisor loop: monitor each worker and restart after crash
    try:
        while True:
            for rank, proc in list(processes.items()):
                if not proc.is_alive():
                    exitcode = proc.exitcode
                    if exitcode != 0:
                        print(f"[Supervisor] Worker {rank} crashed (exitcode={exitcode}). Restarting in 30s...")
                        # Clean up possible core dumps
                        for cf in glob.glob(os.path.join(os.getcwd(), "core.*")):
                            try:
                                os.remove(cf)
                            except Exception:
                                pass
                        ...
                        # Clear any stale token(s) for this rank
                        # (optional: drain results_queue if needed)

                        # Restart the crashed worker
                        processes[rank] = training_process(rank, world_size, shared_array, results_queue)
                    else:
                        # Process exited cleanly (e.g. completed all episodes)
                        print(f"[Supervisor] Worker {rank} finished normally. Not restarting.")
                        del processes[rank]

            # If all workers have completed normally, break
            if not processes:
                print("[Supervisor] All workers have completed. Exiting.")
                break
