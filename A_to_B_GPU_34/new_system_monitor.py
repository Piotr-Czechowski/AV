...
RunMonitor    – one per run; system-wide CPU/memory/swap + CARLA process stats.
                Writes <outdir>/logs/system.jsonl.
WorkerMonitor – one per worker; per-process stats.
                Writes <outdir>/logs/worker_<id>/system.jsonl.

Optional GPU stats: if pynvml is available (bundled with recent NVIDIA
drivers), we also sample GPU utilisation and memory per device.
"""
...
def _sample_system():
    ...
    rec['cpu_percent_mean'] = round(sum(cpu_pct) / len(cpu_pct), 1)
    rec['cpu_percent_max'] = round(max(cpu_pct), 1)
    rec['cpu_count_busy'] = sum(1 for c in cpu_pct if c > 50.0)
    rec['cpu_count_total'] = len(cpu_pct)

    freq = psutil.cpu_freq()
    if freq is not None:
        rec['cpu_freq_mhz'] = round(freq.current, 1)

    vmem = psutil.virtual_memory()
    rec['mem_used_gb'] = round(vmem.used / 1e9, 2)
    rec['mem_available_gb'] = round(vmem.available / 1e9, 2)
    rec['mem_percent'] = vmem.percent

    swap = psutil.swap_memory()
    rec['swap_used_gb'] = round(swap.used / 1e9, 2)
    ...
    for p in psutil.process_iter(['memory_info']):
        try:
            ...
            pass
    rec['total_procs'] = total_procs
    rec['total_rss_gb'] = round(total_rss / 1e9, 2)
    return rec
...
def _sample_worker(pid):
    ...
    try:
        ...
        with proc.oneshot():
            rec['cpu_percent'] = proc.cpu_percent(interval=None)
            mem_info = proc.memory_info()
            rec['rss_gb'] = round(mem_info.rss / 1e9, 3)
            rec['vms_gb'] = round(mem_info.vms / 1e9, 3)
            rec['num_threads'] = proc.num_threads()
            ctx = proc.num_ctx_switches()
            rec['ctx_voluntary'] = ctx.voluntary
            rec['ctx_involuntary'] = ctx.involuntary
    except (psutil.NoSuchProcess, psutil.AccessDenied):
        pass
    return rec
...
class RunMonitor(_BaseMonitor):
    def __init__(self, outdir, interval=10.0, track_carla=True,
                 track_gpu=True):
        log_path = os.path.join(outdir, 'logs', 'system.jsonl')
    ...
    def _sample(self):
        rec = {'system': _sample_system()}
        if self.track_carla:
            rec['carla'] = _sample_carla(self._carla_pids)
        if self.track_gpu and _NVML_AVAILABLE:
            rec['gpus'] = _sample_gpus()
        return rec


class WorkerMonitor(_BaseMonitor):
    def __init__(self, outdir, worker_id, interval=10.0):
    ...
    def _sample(self):
        rec = _sample_worker(self._worker_pid)
