...
class StateObserver:
    ...
    def save_to_disk(self):
        ...
        self.image.save_to_disk(f"Chase/images/{self.episode}/{int(self.step)}.jpeg")
    ...
    def draw_related_values(self, episode=None, step=None):
        ...
        cv2.imwrite(f"Chase/images/{self.episode}/{int(self.step)}_s.jpeg", image_sub)

    def save_together(self):
        
        # Wczytanie dwóch obrazów JPG
        image1 = cv2.imread(f"Chase/images/{self.episode}/{int(self.step)}.jpeg")
        image2 = cv2.imread(f"Chase/images/{self.episode}/{int(self.step)}_s.jpeg")
        ...
        try:
            os.mkdir("Chase/images/combined")
        except FileExistsError:
            pass
        try:
            os.mkdir(f"Chase/images/combined/{self.episode}")
        except FileExistsError:
            pass
        cv2.imwrite(f"Chase/images/combined/{self.episode}/{int(self.step)}_combined.jpeg", combined_image)
