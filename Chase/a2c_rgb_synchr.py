...
# Tensorboard
# #writer = Summary#writer(comment=model_description)
...
class DeepActorCriticAgent(mp.Process):
    ...
    def load(self, name):
        ...
        self.actor_optimizer.load_state_dict(agent_state["actor_optimizer"])
        self.critic_optimizer.load_state_dict(agent_state["critic_optimizer"])
        self.actor.to(device)
        self.critic.to(device)
        try:
            self.mean_reward = agent_state["mean_reward"]
        except:
            pass
        try:
...
def handle_crash(results_queue):
    ...
    while 1:
        ...
        while not done:
            ...
            new_state, reward, done = agent.environment.step()
        ...
        # effective_chase_times.append(agent.env.effective_chase_per)
        # avg_chase_per = sum(effective_chase_times) / len(effective_chase_times)
        #writer.add_scalar("effective_chase%", self.env.effective_chase_per, episode)
        #writer.add_scalar("effective_chase%_mean", avg_chase_per, episode)
    del world
